...
//Anthony note: changing directory path to ./bin_fpga/"%s_v%d.aocx"
inline static char* getVersionedKernelName(const char* kernel_name,
  ...
  // versioning
#if defined(ALTERA)
  snprintf(vname, slen, "./bin_fpga/%s_v%d.aocx", kernel_name, version);
#elif defined(USE_JIT)
  snprintf(vname, slen, "%s_v%d.cl", kernel_name, version);
#else
  // Anthony note: just default to using fpga...
...
inline static char* getVersionedKernelName2(const char* kernel_name,
  ...
  // versioning
#if defined(ALTERA)
  snprintf(vname, slen, "./bin_fpga/%s_%s.aocx", kernel_name, version_string);
#elif defined(USE_JIT)
...
// Anthony note:
// AOCL_ALIGNMENT defined above in this current file
// Does this need to be a static function???
// Anthony note: I've added a 1 to alignedMalloc function declaration (stupid fix) so that the compiler doesn't get confused. Sticking with the HARP defined function instead.

inline static void* alignedMalloc1(size_t size)
{
#ifdef _WIN32
